...
import flow.dao.DsCommonFlowCategoryDao;
import flow.dao.DsCommonFlowDao;
import flow.dao.DsCommonFlowTaskDao;
...
//根据id获取任务
	public DsCommonFlowTask getTask(Long id)
	{
		return (DsCommonFlowTask) taskDao.getByid(id);
	}
	/**
	 * 更新表单
	 * @param ids 流程管理主键数组
	 */
	public void updateFlowDatetable(DsCommonFlow po)
	{
		flowDao.update(po);
	}
	public void updateTaskDatetable(long id,String datatable)
	{
		List<DsCommonFlowTask> a = taskDao.queryList(id);
		for(int i=0;i<a.size();i++) 
		{
		a.get(i).setDatatable(datatable);
		taskDao.update(a.get(i));
		}
